...
static void _slurm_rpc_submit_batch_job(slurm_msg_t * msg)
{
	...
	if (error_code == SLURM_SUCCESS) 
	{
		...
		if (job_desc_msg->job_id != SLURM_BATCH_SCRIPT) 
		{
			...
			if (job_ptr && IS_JOB_FINISHED(job_ptr)) 
			{
				...
				{
					debug("Attempt to re-use active "
		...
		{	/* Active job allocation */
			 ...
			 * load. */
			if (!validate_slurm_user(uid)) {
				debug("Attempt to execute batch job step by "
				     "uid=%d", uid);
				slurm_send_rc_msg(msg, ESLURM_NO_STEPS);
				unlock_slurmctld(job_write_lock);
				_throttle_fini(&active_rpc_cnt);
			...
			if (error_code != SLURM_SUCCESS) {
				debug("_launch_batch_step: %s",
				     slurm_strerror(error_code));
				slurm_send_rc_msg(msg, error_code);
			} else {
				debug("_launch_batch_step StepId=%u.%u %s",
	...
	//verbose("%u",job_desc_msg->job_id);
	//verbose("%u",job_ptr->job_id);

	job_desc_msg->job_id = job_ptr->job_id;
	int rc = job_submit_plugin_submit_first(job_desc_msg, (uint32_t) uid, err_msg);
        if (rc != SLURM_SUCCESS)
                return rc;
	...
	if ((error_code != SLURM_SUCCESS) &&
	    ...
	    (error_code != ESLURM_POWER_NOT_AVAIL) &&
	    (error_code != ESLURM_POWER_RESERVED)) {
		debug("_slurm_rpc_submit_batch_job: %s",
		     slurm_strerror(error_code));
		if (err_msg)
			slurm_send_rc_err_msg(msg, error_code, err_msg);
		else
		...
		verbose("ME_slurm_rpc_submit_batch_job JobId=%u %s",
